...
integer, parameter :: npts1=101, npts2=25
real(8), parameter :: stride1=5d3,stride2=2d3
...
real(8) :: dtout=300d0,tdur=7200d0                  !output rate and total runtime
real(8) :: dt1,dt2
real(8) :: toutnext
...
do while (t<tdur)
  ...
  !FILL GHOST CELLS
  call boundaries(t,x1,x2,rhom,rhov1,rhov2,rhov3,rhoeps,B1,B2,B3,v1,v2,v3)
...
contains
  ...
  subroutine initialization(t,x1,x2,rhom,rhov1,rhov2,rhov3,rhoeps,B1,B2,B3,v1,v2,v3)
    ...
    real(8) :: Ti=1d3                             !reference temperature used to initialize internal energy
    real(8) :: n0=1d20                             !reference number density value
    ...
    do ix2=-1,lx2+2
      do ix1=-1,lx1+2
        H=kb*Ti/amu/abs(g1(ix1,ix2))
        rhom(ix1,ix2)=n0*amu*exp(-(x1(ix1)-x1(1))/H)
      end do
    end do
...
!    end do
  ...
  subroutine boundaries(t,x1,x2,rhom,rhov1,rhov2,rhov3,rhoeps,B1,B2,B3,v1,v2,v3)
    ...
    real(8) :: Ti=1d3                             !temperature of plasma injected from the boundary
    real(8) :: n0=1d20                            !number density reference value
...
!    rhom(-1:0,1:lx2)=spread(rhom(1,1:lx2),1,2)              !bottom
    rhom(-1:0,1:lx2)=n0*amu
!    rhom(lx1+1:lx1+2,1:lx2)=spread(rhom(lx1,1:lx2),1,2)     !top
    rhom(lx1+1,1:lx2)=rhom(lx1-1,1:lx2)/rhom(lx1-2,1:lx2)*rhom(lx1,1:lx2)
    rhom(lx1+2,1:lx2)=rhom(lx1-1,1:lx2)/rhom(lx1-2,1:lx2)*rhom(lx1+1,1:lx2)
...
!    rhov1(lx1+1:lx1+2,1:lx2)=spread(rhov1(lx1,1:lx2),1,2)
    rhov1(lx1+1:lx1+2,1:lx2)=rhom(lx1+1:lx1+2,1:lx2)*spread(v1(lx1,1:lx2),1,2)
    rhov1(lx1+1:lx1+2,1:lx2)=max(rhov1(lx1+1:lx1+2,1:lx2),0d0)
...
!    rhoeps(-1:0,1:lx2)=spread(rhoeps(1,1:lx2),1,2)
    rhoeps(-1:0,1:lx2)=rhom(-1:0,1:lx2)*kb*Ti/amu/(gammap-1d0)
!    rhoeps(lx1+1:lx1+2,1:lx2)=spread(rhoeps(lx1,1:lx2),1,2)
    rhoeps(lx1+1:lx1+2,1:lx2)=rhom(lx1+1:lx1+2,1:lx2)*kb*Ti/amu/(gammap-1d0)
