...
integer, parameter :: npts1=1001, npts2=25
real(8), parameter :: tcfl=0.75d0

real(8), parameter :: stride1=3d6,stride2=3d6
...
real(8) :: dtout=300d0,tdur=24*3600d0                  !output rate and total runtime
real(8) :: dt1,dt2
real(8) :: toutnext
...
call gravity(x1,x2)                    !comment out to run without gravity
...
contains
  ...
  subroutine initialization(t,x1,x2,rhom,rhov1,rhov2,rhov3,rhoeps,B1,B2,B3,v1,v2,v3)
    ...
    real(8) :: n0=1d17                            !chromospheric number density value
    real(8) :: B0=0d0
    real(8) :: r,H
...
!    rhom=n0*amu
    do ix1=1,lx1
      do ix2=1,lx2
        r=x1(ix1)-x1(1)+Rsun
!        rhom(ix1,ix2)=n0*amu*exp(Gconst*Msun*amu/2d0/kb/Ti*(1d0/r-1d0/Rsun))
        H=kb*Ti/amu/abs(g1(ix1,ix2))
        rhom(ix1,ix2)=n0*amu*exp(-(r-Rsun)/H)
      end do
    end do
  ...
  subroutine boundaries(t,x1,x2,rhom,rhov1,rhov2,rhov3,rhoeps,B1,B2,B3)
    ...
    real(8) :: n0=1d17                            !number density reference value
    real(8) :: B0=1d-9                            !magnetic field reference value
...
!    rhov2(1:lx1,-1:0)=rhom(1:lx1,-1:0)*vsw
